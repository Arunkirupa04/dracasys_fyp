...
export default function Module3Card({ data, history = [], config }) {
  const mod = moduleConfig(config, 'm3')
  const result = data.result
  const isAnom = result?.is_anomaly ?? false
  const mse = result?.reconstruction_mse
  const threshold = result?.threshold ?? parseFloat(mod?.validation_metrics?.find((m) => m.label.includes('Threshold'))?.value || '0.098')
      ...
      {result == null ? (
        ...
        <>
          <div style={styles.metricsRow}>
            <Metric label="Status" value={isAnom ? 'ANOMALY' : 'Normal'} alert={isAnom} />
            <Metric label="Reconstruction MSE" value={mse?.toFixed(5)} highlight={isAnom} />
          ...
          <div style={{ ...styles.alertBadge, background: isAnom ? 'rgba(239,68,68,0.15)' : 'rgba(34,197,94,0.12)', borderColor: isAnom ? 'rgba(239,68,68,0.4)' : 'rgba(34,197,94,0.3)', color: isAnom ? 'var(--red)' : 'var(--green)' }}>
            <span>{isAnom ? '⚠ System Anomaly Detected' : '✓ Normal Behavior'}</span>
            {isAnom && result.anomaly_type && <span style={styles.anomalyType}> — {result.anomaly_type}</span>}
          </div>

          <div style={styles.scoreSection}>
            <div style={styles.scoreHeader}>
              <span style={styles.scoreLabel}>MSE vs Threshold</span>
              <span style={{ ...styles.scoreVal, color: barColor }}>{mse?.toFixed(5)}</span>
            </div>
            <div style={styles.barTrack}>
              <div style={{ ...styles.barFill, width: `${fillPct}%`, background: barColor }} />
              <div style={{ ...styles.thresholdMarker, left: '50%' }} title={`Threshold: ${threshold}`} />
            </div>
            <div style={styles.thresholdLabel}>Threshold: {threshold.toFixed(5)}</div>
          </div>

          <div style={styles.chartGrid}>
            {history.length > 0 && (
              <ScoreTrendChart
                title="Reconstruction MSE Across Streamed Samples"
                points={history.map((r) => ({ sample: r.sample, value: r.reconstruction_mse, isAnomaly: r.is_anomaly }))}
                threshold={threshold}
                scale="log"
                width={640}
                height={260}
...
function Placeholder({ mod }) {
  return (
    <div style={styles.placeholder}>
      ...
      <div style={styles.phSub}>Window: {mod?.inputs?.window_shape} · {mod?.inputs?.interval}</div>
    </div>
  )
}
...
const styles = {
  ...
  anomalyType: { fontWeight: 500, fontStyle: 'italic' },
  ...
  barTrack: { position: 'relative', height: 10, background: 'var(--surface2)', borderRadius: 5 },
  barFill: { height: '100%', borderRadius: 5, transition: 'width 0.6s ease' },
  thresholdMarker: { position: 'absolute', top: -3, width: 2, height: 16, background: 'var(--yellow)', borderRadius: 1 },
